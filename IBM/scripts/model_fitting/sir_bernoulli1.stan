...
parameters{
  ...
  real<lower=0> zz[J]; //varying intercept for each tray
  real<lower=0> lambda;
  real<lower=0> kappa;
}
...
model{
  ...
  beta ~ beta(1,5);  
  zz ~ gamma(lambda, kappa); //needs to be positive. not sure if exp is the best. 
 ...
 for(j in 1:J){
   for(T in 1:(t-1)){
     for(i in 1:N){
       ...
       p_ci = 1 - exp(-(alpha + beta*sum(I[,T,j]) + zz[ID[j]] ));
       z_ci[i,T,j] ~ bernoulli(p_ci); 
       }
       
       if(sum(I[,T,j]) > 0 && S[i,T,j]){ //for susceptible
       p_si = 1 - exp(-(beta*sum(I[,T,j]) + zz[ID[j]] ));
