...
#plants were inoculated 4 days after sowing. inoculum has 2 days to be colonized in PDA plates. 
library(forcats)
library(tidyverse)
library(readxl)
...
ID <- read_xlsx('GH_data/soil_primary_trans.xlsx', sheet = 1)
print(ID, width = Inf)

#merge the position ID table with the data on germination and infection status
pos <- read_xlsx('GH_data/soil_primary_trans.xlsx', sheet=2, na = "NA")
germ <- read_xlsx('GH_data/soil_primary_trans.xlsx', sheet=3, na = "NA")
inf <- read_xlsx('GH_data/soil_primary_trans.xlsx', sheet=4, na = "NA")
...
df_germ %>% 
  filter(!is.na(germ)) %>% 
  ggplot(., aes(germ, prop, color = as.factor(soil))) +
  geom_point() +
  geom_line() +
  facet_grid(~species)
#plot2
...
df_inf %>% 
  filter(!is.na(inf)) %>% 
  ggplot(., aes(inf, prop, color = soil)) +
  geom_point() +
  geom_line() +
  facet_grid(~species)
