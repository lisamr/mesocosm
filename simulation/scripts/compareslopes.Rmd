...
ggplot(filter(dat, species=="tot", time==20), aes(rich, (pI)))+
  ...
  facet_wrap(~dens+rand)
#t=40
ggplot(filter(dat, species=="tot", time==40), aes(rich, (pI)))+
  geom_point()+
  geom_line(aes(group=interaction( rep)))+
  facet_wrap(~dens+rand)
```
...
```{r, warning=F}
dat40 <- dat %>% 
  filter(time==40, species=="tot") %>%  #look at total infections at time final
...
#look at data
ggplot(dat40, aes(rich, (pI), color=interaction(rand, dens), group=interaction(rand, dens)))+
  geom_point()+
...
m1 <- glm(cbind(n.I, n.S) ~ dens*rand*rich, data = dat40, family = binomial)
#summary(m1)

m2 <- glm(cbind(n.I, n.S) ~ trt*rich, data = dat40, family = binomial)
#summary(m2)
#summary(m2)
...
m2.1 <- glm(cbind(n.I, n.S) ~ (trt+rep2)*rich, data = dat40, family = binomial) #slope varies with each block.
m2.2 <- glm(cbind(n.I, n.S) ~ trt*rich + rep2, data = dat40, family = binomial) #just intercept varies with each block.
m2.3 <- glm(cbind(n.I, n.S) ~ trt*rich + rep3, data = dat40, family = binomial) #rep3 assigns all of the deterministic blocks as the same.
m2.4 <- glm(cbind(n.I, n.S) ~ (trt+rep3)*rich, data = dat40, family = binomial)
...
dat40$pred <- pred
ggplot(dat40, aes(rich, pI, group=rep2))+
  geom_point()+
  geom_line()+
...
```{r}
ggplot(dat40, aes(rich, (n.I), color=interaction(rand, dens), group=interaction(rand, dens)))+
  geom_point()+
  geom_smooth(method='lm')#+
  #facet_wrap(~dens+rand)

m3 <- MASS::glm.nb(n.I ~ trt*rich, data=dat40)
#summary(m3)
...
#proportion infected
require(nlme)
m <- lmList(pI ~ rich | rep2, data=dat40)
cm <- coefficients(m)
slopes.pI <- data.frame(rep2 = row.names(cm), slopes.pI = cm[,2])
dat40 <- left_join(dat40, slopes.pI)
head(dat40)

datslopes <- dat40 %>% filter(!duplicated(rep2))
...
#total infected
m <- lmList(n.I ~ rich | rep2, data=dat40)
cm <- coefficients(m)
slopes.nI <- data.frame(rep2 = row.names(cm), slopes.nI = cm[,2])
dat40 <- left_join(dat40, slopes.nI)
head(dat40)
...
#incidence
wilcox.test(datslopes$slopes.pI[datslopes$trt=="sub.FALSE"])#.002
wilcox.test(datslopes$slopes.pI[datslopes$trt=="sub.TRUE"])#1
wilcox.test(datslopes$slopes.pI[datslopes$trt=="add.FALSE"])#.002
wilcox.test(datslopes$slopes.pI[datslopes$trt=="add.TRUE"])#.084

wilcox.test(datslopes$slopes.nI[datslopes$trt=="sub.FALSE"])#.002
wilcox.test(datslopes$slopes.nI[datslopes$trt=="sub.TRUE"])#1
...
```{r}
ggplot(dat40, aes(rich, pI, group=interaction(rand, dens)))+
  geom_point()+
  geom_line(aes(group=rep))+
  facet_wrap(~dens+rand)
ggplot(dat40, aes(rich, n.I, group=interaction(rand, dens)))+
  geom_point()+
  geom_line(aes(group=rep))+
  facet_wrap(~dens+rand)
