...
knitr::opts_knit$set(root.dir = '/Users/lisarosenthal/Box/mesocosm expt/mesocosm.git')
...
D.sub <- 2
getabund4 <- function(dist, L=50, rand=F){
  #load function
...
prepdata2 <- function(dens, rand, L, design){
  #dens="add" or "sub", rand=F or T
  D <- design %>% filter(d==dens) %>% pull(r)#(planting distances)
  A <- getabund4(D,L, rand) 
  list(A=A, D=D, dens=dens, rand=rand)
}

#simulation and results functions ("f.sim" below)
results3 <- function(f.sim=s2, t=20, B, L=50, nreps=10, density="sub", Random=F, design){
  #density="add" or "sub", Rand=T or F, f.sim=s2
  R <- c(1,2,4,6)
  ...
  bres2 <- list(NULL)
  for(i in 1:nreps){
    data <- prepdata2(density, Random,L, design)
...
fb3 <- function(Beta, t, L=50, n=10, design){
  SD <-  results3(s2,t,L, B=Beta, nreps=n, "sub", F, design) #substitutive, deterministic
  SR <-  results3(s2,t,L, B=Beta, nreps=n, "sub", T, design) #substitutive, random
  AD <-  results3(s2,t,L, B=Beta, nreps=n, "add", F, design) #additive, deterministic
  AR <-  results3(s2,t,L, B=Beta, nreps=n, "add", T, design) #additive, random
  response <- rbind(SD$responses, SR$responses, AD$responses, AR$responses)
  response.summary <- rbind(SD$response.summary, SR$response.summary, AD$response.summary, AR$response.summary)
